...
import { AlertCircle, FileTerminal, ShieldAlert, Save, Bot, Users, Battery, Loader2, CheckCircle2 } from "lucide-react";
import {
    LineChart,
    Line,
    CartesianGrid,
...
const AiReportPopup = ({ status, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[100]">
            <div className="bg-slate-900 text-white p-8 rounded-2xl shadow-xl max-w-sm w-full text-center space-y-4 flex flex-col items-center">
                <div className={`p-4 rounded-full ${status === 'loading' ? 'bg-indigo-500' : 'bg-green-500'}`}>
                    {icon}
                </div>
                <h2 className="text-xl font-bold">{message}</h2>
                <p className="text-sm text-slate-400">
                    {status === 'loading'
                        ? "This may take a moment while the AI analyzes the simulation data."
                        : "Opening the report in a new tab now."}
                </p>
            </div>
        </div>
    );
...
const SimulationDashboard = () => {
    ...
    // New state for popup
    const [reportStatus, setReportStatus] = useState("hidden"); // "hidden", "loading", "done"
    ...
    useEffect(() => {
        ...
        if (simulationRunning) {
            setAiLoading(false);
            setLogHash(null);
            setReportStatus("hidden"); // Reset popup on start
    ...
    // --- MODIFIED: On stop, show popup, delay, then generate and open report in new tab ---
    useEffect(() => {
        if (!simulationRunning && simStartTimeRef.current !== null) {
            setAiLoading(true);
            setReportStatus("loading"); // Show loading popup

            // Wait for 10 seconds before generating the report
            const delayReportGeneration = setTimeout(() => {
                try {
                    const reportData = {
                        workerData: workers,
                    ...
                    setReportStatus("done"); // Change popup to success state

                    // Hide popup after a brief moment
                    setTimeout(() => {
                        setReportStatus("hidden");
                    }, 2000);

                } catch (e) {
                    toast.error("Failed to generate report.");
                    setReportStatus("hidden"); // Hide popup on error
                    console.error(e);
                } finally {
                    setAiLoading(false);
            ...
            return () => clearTimeout(delayReportGeneration);
        }
    }, [simulationRunning, workers, incidents, carbonFootprint]);

    return (
        <>
            <AiReportPopup status={reportStatus} />
