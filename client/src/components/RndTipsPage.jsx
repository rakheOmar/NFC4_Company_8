...
const InsightCard = ({ icon, title, value, description, status }) => {
  // Orange for 'good' or 'warning', gray for 'neutral'
  const statusColors = {
    good: "border-orange-500 bg-orange-50",
    warning: "border-orange-500 bg-orange-50", // Warning is also highlighted
  ...
  useEffect(() => {
    ...
    if (storedProductionData && storedImportData) {
      ...
      const prodInsight = {
        ...
        status: productionAchievement >= 15 ? "good" : "warning", // Adjusted threshold for demo data
      };
      ...
      const offtakeInsight = {
        ...
        status: offtakeAchievement >= 15 ? "good" : "warning", // Adjusted threshold for demo data
      };
      ...
      const companyProduction = productionData
        ...
        .map((d) => ({
          ...
          fill: COLORS[Math.floor(Math.random() * COLORS.length)], // Assign random color from palette
        }));
